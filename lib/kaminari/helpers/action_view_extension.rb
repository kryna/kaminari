module Kaminari
  ...
  module ActionViewExtension
    ...
    def link_to_previous_page(scope, name, options = {}, &block)
      ...
      link_to_unless scope.previous_page.nil?, name, params.merge(param_name => scope.previous_page), options.reverse_merge(:rel => 'previous') do
        block.call if block
      end
    end
    ...
    def link_to_next_page(scope, name, options = {}, &block)
      params = options.delete(:params) || {}
      param_name = options.delete(:param_name) || Kaminari.config.param_name
      link_to_unless (scope.total_pages == scope.current_page), name, params.merge(param_name => scope.next_page), options.reverse_merge(:rel => 'next') do
        block.call if block
      end
    end
